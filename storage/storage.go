...
func Init(workspace string, db_config *config.ConfigDb) {
	...
	case "mysql":
		dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s&parseTime=True&loc=Local",
			...
			db_config.Mysql.Charset,
		)

		if db_config.Mysql.TLS != "" {
			dsn += "&tls=" + db_config.Mysql.TLS
		}
...
func GetStatsForShowByTgId(tg_id int64) ([]*StatsForShow, error) {
	tmp := make([]*StatsForShow, 0)
	select_str := fmt.Sprintf("select %v.alias as app_alias, %v.ms_username as user_alias, %v.alias as op_alias, %v.id as op_id, %v.tg_id, %v.success, %v.failure, %v.last_time",
		Table_APPs, Table_Users, Table_Op, Table_Op, Table_Users, Table_Stats, Table_Stats, Table_Stats)
...
func GetAllStatsForShow() ([]*StatsForShow, error) {
	tmp := make([]*StatsForShow, 0)
	select_str := fmt.Sprintf("select %v.alias as app_alias, %v.ms_username as user_alias, %v.alias as op_alias, %v.id as op_id, %v.tg_id, %v.success, %v.failure, %v.last_time",
		Table_APPs, Table_Users, Table_Op, Table_Op, Table_Users, Table_Stats, Table_Stats, Table_Stats)
	select_str = fmt.Sprintf("%v from %v, %v, %v, %v", select_str, Table_APPs, Table_Users, Table_Op, Table_Stats)
